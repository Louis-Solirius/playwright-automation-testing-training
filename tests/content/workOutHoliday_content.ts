const workedPerWeek_content = {
    pageTitle: "Do you want to work out holiday:",
    pageTitle2: "Do you want to calculate the holiday:",
    radio1: "for a full leave year",
    radio2: "for someone starting part way through a leave year",
    radio3: "for someone leaving part way through a leave year",
    radio4: "for someone starting and leaving part way through a leave year",
} as const;

export default  workedPerWeek_content
